...
// line 33 "ce204-hw2-Treatment.ump"
// line 97 "ce204-hw2-Treatment.ump"
public class Presscription
{
  ...
  public Presscription(Patient aPatient)
  {
    therapies = new ArrayList<Therapy>();
    boolean didAddPatient = setPatient(aPatient);
    if (!didAddPatient)
    {
      throw new RuntimeException("Unable to create presscription due to patient. See http://manual.umple.org?RE002ViolationofAssociationMultiplicity.html");
    }
  ...
  public boolean addTherapy(Therapy aTherapy)
  {
    ...
    Presscription existingPresscription = aTherapy.getPresscription();
    boolean isNewPresscription = existingPresscription != null && !this.equals(existingPresscription);
    if (isNewPresscription)
    {
      aTherapy.setPresscription(this);
    }
    else
    {
  ...
  public boolean removeTherapy(Therapy aTherapy)
  {
    ...
    //Unable to remove aTherapy, as it must always have a presscription
    if (!this.equals(aTherapy.getPresscription()))
    {
  ...
  public boolean setPatient(Patient aPatient)
  {
    ...
    {
      existingPatient.removePresscription(this);
    }
    patient.addPresscription(this);
    wasSet = true;
    return wasSet;
  }
  /* Code from template association_MinimumNumberOfMethod */
  ...
  public boolean addDoctor(Doctor aDoctor)
  {
    ...
    doctors.add(aDoctor);
    if (aDoctor.indexOfPresscription(this) != -1)
    {
      wasAdded = true;
    }
    else
    {
      wasAdded = aDoctor.addPresscription(this);
      if (!wasAdded)
      {
  ...
  public boolean removeDoctor(Doctor aDoctor)
  {
    ...
    {
      wasRemoved = aDoctor.removePresscription(this);
      if (!wasRemoved)
      {
  ...
  public boolean addDiagnosi(Diagnosis aDiagnosi)
  {
    ...
    if (aDiagnosi.indexOfPresscription(this) != -1)
    {
      wasAdded = true;
    }
    else
    {
      wasAdded = aDiagnosi.addPresscription(this);
      if (!wasAdded)
      {
  ...
  public boolean removeDiagnosi(Diagnosis aDiagnosi)
  {
    ...
    {
      wasRemoved = aDiagnosi.removePresscription(this);
      if (!wasRemoved)
      {
